...
?>
    <h2>Покрытие дисциплины стандартом </h2>
...
foreach ($prof as $pr) {
    ...
    foreach ($general->where(['id_profession'=>$pr->id])->all() as $gen){
        ...
        foreach ($workfun->where(['id_general'=>$gen->id])->all() as $wf){
            ...
            for($i=0;$i<3;$i++){
                if($i==0){
                    $resa = \Yii::$app->db->createCommand("CALL coverage_dis_sort(@r,:sort,:workfun,:dis);")
                        ->bindValue(':sort' , 1)
                        ->bindValue(':workfun' , $wf->id)
                        ->bindValue(':dis' , $discipline->id)->execute();
                    ...
                    $ress = \Yii::$app->db->createCommand("CALL coverage_dis_sort(@r,:sort,:workfun,:dis);")
                        ->bindValue(':sort' , 3)
                        ->bindValue(':workfun' , $wf->id)
                        ->bindValue(':dis' , $discipline->id)->execute();
                    ...
                    $resk = \Yii::$app->db->createCommand("CALL coverage_dis_sort(@r,:sort,:workfun,:dis);")
                        ->bindValue(':sort' , 2)
                        ->bindValue(':workfun' , $wf->id)
                        ->bindValue(':dis' , $discipline->id)->execute();
            ...
            $resw = \Yii::$app->db->createCommand("CALL coverage_dis_workfun(@r,:workfun,:dis);")
                ->bindValue(':workfun' , $wf->id)
                ->bindValue(':dis' , $discipline->id)->execute();
        ...
        $resg = \Yii::$app->db->createCommand("CALL coverage_dis_general(@r,:general,:dis);")
            ->bindValue(':general' , $gen->id)
            ->bindValue(':dis' , $discipline->id)->execute();
    ...
    $resp = \Yii::$app->db->createCommand("CALL coverage_dis_prof(@r,:prof,:dis);")
        ->bindValue(':prof' , $pr->id)
        ->bindValue(':dis' , $discipline->id)->execute();
